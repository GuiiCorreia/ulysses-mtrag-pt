...
faith = {m: [u["faith"] for u in gen[m]["per_unit"]] for m in gen}
for m in sorted(gen, key=lambda x: -mean(faith[x])):
...
q3 = [m for m in gen if "qwen3-8b" in m.lower()][0]
g31 = [m for m in gen if "gemma-4" in m.lower()][0]
