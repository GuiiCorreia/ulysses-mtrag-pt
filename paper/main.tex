...
per-turn degradation and a \emph{retriever-dependent} optimum: dense retrieval does
no better with history than with the current turn alone, while lexical BM25 is
significantly best with the full dialogue history, which acts as query expansion; (ii) run a
...
relevance judge against the pre-existing expert labels: full-pool agreement is
modest ($0.58$), dominated by pooled documents the experts never assessed, and
rises to $0.94$ on the subset they explicitly labeled. We will release the
benchmark, prompts, and code; an anonymized repository accompanies this submission.
\end{abstract}
...
\begin{itemize}
  ...
  statistically indistinguishable from a 72B model nine times its size, so scaling
  up is not a reliable route to faithful legal RAG here, and
...
query-relevant passages \cite{Chen2024BGEM3,Zhou2024LengthCollapse}.
...
\label{sec:benchmark}
\textbf{Construction.} We seed sessions from the 692 queries of the public
Ulysses-RFCorpus release, each carrying graded relevance judgments (relevant /
partially relevant / irrelevant) produced by the legislative consultants of the
Brazilian Chamber of Deputies \cite{Vitorio2024}, over a corpus of 105{,}669
...
\cite{Vitorio2024}.} We first filter to queries
...
prompted to produce a coherent next question of a \emph{prescribed}
type---follow-up, clarification, non-standalone, comparative, or temporal---a
scheme we designed around the conversational challenges MTRAG emphasizes
(non-standalone and comparative questions, answerability) \cite{Katsis2025MTRAG}.
...
\cite{Mao2023LLM4CS,Elgohary2019CANARD}. Per turn and per retriever, we pool the documents retrieved by the four strategies
and judge the pool once (standard pooling), scoring each strategy with nDCG@10 on
the shared labels; a persistent judgment cache assigns identical labels to any
(query, document) pair that appears in both retrievers' pools. We run the full strategy
...
to span the size range at competitive quality---Qwen2.5-7B \cite{Qwen2024Qwen25},
Qwen3-8B \cite{Yang2025Qwen3}, Ministral-8B, Phi-4 (14B) \cite{Abdin2024Phi4}, and
Gemma-4-31B---so that the effect of scale can be read directly from the comparison. We score each answer on three axes adapted from MTRAG's
...
and prevalence-adjusted PABAK \cite{Byrt1993PABAK}. Over the full pool ($n{=}1000$ query--document pairs),
...
abstention, and a dedicated human annotation of the full retrieved pool to tighten
the judge-reliability bound of RQ2.
...
well-chosen 8B model is competitive with a 72B nine times larger (difference not
significant)---with retrieval degradation propagating to answers only weakly; and
