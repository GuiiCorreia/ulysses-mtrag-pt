...
% PENDING (slots marked with %%PENDING): (a) BM25 robustness run,
% (b) anonymized repository URL, (c) human-validation kappa (RQ2).
% ============================================================
...
a synthetic multi-turn RAG benchmark for Brazilian legislative dialogue, built by
anchoring the first turn of each session in real expert queries carrying human
relevance judgments (Ulysses-RFCorpus) and generating coherent follow-up turns with
an LLM; relevance of retrieved documents is assessed by an LLM judge distinct from
the generator. Using the benchmark we (i) measure how retrieval quality evolves
across turns under four context strategies and two retrievers, finding significant
per-turn degradation and that the best strategy is \emph{retriever-dependent}:
last-turn retrieval is best for dense embeddings, whereas appending the dialogue
history is significantly best for lexical BM25, which uses it as query expansion; (ii) run a
size- and family-controlled generation study (7B to 72B parameters), finding that
model family dominates parameter count for faithfulness---a well-chosen 8B model is
more faithful than 14B and 31B models from other families and competitive with a
72B model nine times its size---while retrieval degradation propagates to generation
only weakly (Spearman $\rho\!\le\!0.21$); and (iii) assess the LLM
relevance judge against the pre-existing expert labels, where agreement reaches
$0.94$ on the subset the experts explicitly labeled. We release the benchmark,
prompts, and code.
\end{abstract}
...
our construction couples scalability with ecological validity: each session is
anchored in a \emph{real} expert query that carries human relevance judgments, and
only the follow-up turns are LLM-generated. This human-anchored design is what
distinguishes Ulysses-MTRAG from MTRAG \cite{Katsis2025MTRAG}, whose dialogues are
fully human-authored (accurate but unscalable), and from CORAL \cite{Cheng2025CORAL},
whose conversations are derived automatically from Wikipedia (scalable but without
expert relevance ground truth); anchoring in pre-existing expert judgments uniquely
allows us to audit the LLM relevance judge against experts \emph{inside} the
benchmark itself. Our contributions are:
\begin{itemize}
  ...
  best 8B model outperforms 14B and 31B models from other families and matches a
  72B model, so scaling up is not a reliable route to faithful legal RAG here, and
  retrieval degradation propagates to generation only weakly.
...
known biases---position and verbosity \cite{Wang2024Unfair}, self-preference
\cite{Panickssery2024SelfPref}---which motivates our use of a judge distinct from
the generator. Relevance labels are also highly prompt-sensitive
...
pipelines tend to produce decontextualized, self-contained questions
\cite{Choi2021Decontextualization,Vlachos2025Synthetic}---a tendency our benchmark
lets us quantify and stress-test. Degradation over long dialogues is also
...
\cite{Dai2019Deeper,Karpukhin2020DPR,Jiang2024LongRAG}. Embedding models have hard
token limits (BGE-M3: 8192 tokens \cite{Chen2024BGEM3}) and degrade on long inputs
\cite{Liu2024LostMiddle,Zhou2024LengthCollapse}; BM25 remains a strong baseline,
...
and dense BGE-M3 \cite{Chen2024BGEM3}. For the benchmark corpus---legislative bills,
whose official summaries (\emph{ementas}) are short---we embed and index each bill's
summary, which fits comfortably within the encoder's window; the multi-turn
experiments therefore use whole-summary dense indexing.

\textbf{A length boundary and its mitigation.} Before the multi-turn analysis we
characterize one boundary of this stack on Brazilian legal text, observed on the
regulatory sub-domain of JU\'A \cite{Pereira2026JUA} (NormasTCU; $\sim$12k-token
norms, $n{=}46$ queries). There, whole-document dense retrieval \emph{collapses}:
no relevant norm appears in any top-10, yielding nDCG@10$=$0.000 across all
queries, while lexical BM25 attains 0.328 (Table~\ref{tab:chunking}). The mechanism
is length: each norm far exceeds BGE-M3's 8192-token window, so the single vector
indexed per document is computed from a truncated prefix that omits the
query-relevant passages \cite{Chen2024BGEM3,Liu2024LostMiddle,Zhou2024LengthCollapse}.
Applying the standard passage-level remedy \cite{Dai2019Deeper,Karpukhin2020DPR}---
splitting each norm into sub-8k chunks, indexing chunks, and mapping retrieved
chunks back to documents---recovers dense retrieval to parity with BM25
(nDCG@10$=$0.330; BM25 retains better recall). We report this as a quantified
sanity check that motivates our indexing choices, not as a novel technique: bills
never hit this boundary, ultra-long norms do.
...
\label{sec:benchmark}
\textbf{Construction.} We seed sessions from the 692 Ulysses-RFCorpus queries, each
carrying graded relevance judgments (relevant / partially relevant / irrelevant)
produced by the legislative consultants of the Brazilian Chamber of Deputies
\cite{Vitorio2024}, over a corpus of 105{,}669 bills. We first filter to queries
with at least three (partially) relevant bills, embed the query texts with BGE-M3,
and cluster them with $k$-means ($k{=}100$, fixed seed), selecting one
representative seed per cluster to obtain a diverse, non-redundant seed set. The
seed becomes Turn~1 and inherits its expert labels. Turns~2--4 are generated by an
instruction-tuned LLM (Qwen2.5-72B) conditioned on the full session history and
prompted to produce a coherent next question of a \emph{prescribed} type---
follow-up, clarification, non-standalone, comparative, or temporal---sampled to
mirror the turn-type taxonomy and distribution of MTRAG \cite{Katsis2025MTRAG}.
The generator is instructed to stay on-topic and not repeat earlier turns; we
discard degenerate generations (empty, duplicated, or off-topic) and regenerate.
The result is 100 four-turn sessions (400 turns); Fig.~\ref{fig:example} shows one.
A Portuguese-specialized generator (e.g., Gaia/Sabi\'a) is left for future work.
...
explicitly non-standalone, so the benchmark---like other synthetic multi-turn
pipelines \cite{Choi2021Decontextualization,Vlachos2025Synthetic}---consists
largely of self-contained questions on a shared topic. We treat this as a
measurable property of the MTRAG-style synthetic recipe and stress-test its
...
\cite{Mao2023LLM4CS,Elgohary2019CANARD}. Per turn we pool the retrieved documents
across strategies and retrievers and judge them once (standard pooling), then score
each strategy with nDCG@10 on the pooled judgments. We run the full strategy
comparison under \emph{both} retrievers---dense BGE-M3 and lexical BM25
(\S\ref{sec:foundation})---to test whether conclusions about dialogue history are
retriever-specific.
...
prompt, to five open-weight instruction-tuned small language models (SLMs) chosen
to span the size range at competitive quality---Qwen2.5-7B, Qwen3-8B, Ministral-8B,
Phi-4 (14B), and Gemma-4-31B---so that the effect of scale can be read directly
from the comparison. We score each answer on three axes adapted from MTRAG's
generation evaluation \cite{Katsis2025MTRAG}: (i) \emph{context faithfulness}
(reference-less, RAGAS-style \cite{Es2024RAGAS}), judged by Llama-3.3-70B;
(ii) \emph{reference similarity} to an oracle answer via BERTScore-F1
...
BERTScore uses multilingual BERT. Models are accessed through OpenAI-compatible
APIs. All prompts (turn generation, query rewriting, relevance rubric,
faithfulness judging), model identifiers, and configuration files are included in
...
query expansion, supplying lexically matching legal terms. The common ``just use the
last turn'' heuristic is thus safe only for dense retrieval; lexically the opposite
holds. This interaction cautions against drawing conclusions about conversational
context from a single retriever.
...
$0.64$--$0.73$) and history-aware dense strategies do not recover them, consistent
with the decontextualization tendency of LLM-generated questions
\cite{Choi2021Decontextualization,Vlachos2025Synthetic}; contrast ORConvQA
...
position (100 conversations; a single LLM-judged pool per turn scores all strategies,
including T1, so strategies coincide at T1). Best strategy per retriever (all-turn
mean) in bold. The ordering \emph{reverses} between dense and lexical retrieval.}
\label{tab:mtrag}
\centering
...
We audit the LLM judge against the pre-existing expert labels on the Turn-1 pool,
reporting raw agreement alongside Cohen's~$\kappa$ and prevalence-adjusted PABAK
\cite{Byrt1993PABAK}. Over the full pool ($n{=}1000$ query--document pairs),
agreement is modest (raw $0.58$, $\kappa{=}0.27$): the judge labels $69\%$ of
...
(PABAK $0.875$). On this subset $\kappa$ remains moderate ($0.39$) because
...
across the Qwen models faithfulness rises $0.782$ (7B) $\to 0.808$ (8B) $\to 0.841$
...
times its size; raw scale is an unreliable lever relative to family and training.
...
MTRAG-style synthetic construction: follow-ups are largely self-contained
...
time, which we leave as future work alongside eliciting unanswerable turns
($99.3\%$ of ours are answerable), which MTRAG \cite{Katsis2025MTRAG} uses to test
abstention.

\textbf{Validity threats.} (i) We evaluate two retrievers (dense and lexical) and
find the strategy ranking reverses between them; hybrid retrieval and other dense
encoders may behave differently again, so the specific rankings should not be
over-generalized beyond the two families tested.
(ii) The follow-up generator, the oracle, and two evaluated models are related
(same model or family); we mitigate by resting headline claims on the
reference-less faithfulness axis with a judge from a different family, and by
noting that a non-family model leads the reference-based metrics. (iii) The cross-family
comparison has one model per size point, so family and size are partly confounded;
we add a within-family Qwen ladder (7B/8B/72B) to isolate scale, but the 72B anchor
is also the question author, an advantage we cannot fully remove. (iv) Judge-based scores vary across judging runs
($\approx0.05$ in absolute value, ranking-stable) and are LLM-judged over a
retrieval pool, so absolute values should be read as comparative rankings, with
reliability bounded in RQ2. (v) The benchmark covers one legislative corpus,
four-turn sessions, and synthetic follow-ups; human-authored legal dialogue may
behave differently \cite{Qu2020ORConvQA}.

% ============================================================
\section{Conclusion}
We presented Ulysses-MTRAG, a human-anchored synthetic multi-turn RAG benchmark
for Brazilian legislative dialogue, and used it to show that (i) retrieval
degrades significantly with dialogue depth, and the best way to use dialogue history
is retriever-dependent---inert for dense retrieval but a significant gain for lexical
BM25;
(ii) generation faithfulness is governed more by model family than by scale---a
well-chosen 8B model matches a 72B nine times larger---with retrieval degradation
propagating to answers only weakly; and
(iii) the LLM relevance judge agrees with expert labels on $94\%$ of the documents
experts explicitly assessed, with full-pool disagreement dominated by label
incompleteness. The benchmark, prompts, and code will be released; an anonymized
repository accompanies the review.
