...
The seed queries, the bills and the experts' relevance labels come from Ulysses-RFCorpus (Vitório et al., *Language Resources and Evaluation* 59:1257–1277, 2025) and are redistributed here with attribution to that release, as authorised by one of its authors; see `NOTICE`. `data/` carries the subset the 100 sessions draw on; the complete collection (105,669 bills, 1.7 GB) and the full expert-label file are mirrored, unmodified, as a Hugging Face dataset: https://huggingface.co/datasets/GuiiCorreia/ulysses-rfcorpus-mtrag. NormasTCU (Table I only): https://huggingface.co/datasets/ufca-llms/normas-tcu.
